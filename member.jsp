...
<%
    ...
    Object memberId = session.getAttribute("m_id");
    String mName = (String) session.getAttribute("m_name");
    String mEmail = (String) session.getAttribute("m_email");
    String mPhone = (String) session.getAttribute("m_phone");
    String mAddress = (String) session.getAttribute("m_address");
%>
<!DOCTYPE html>
<html lang="zh-TW">
<head>
...
<section class="page-hero">
  ...
  <p class="page-hero-sub">管理會員資料與訂單狀態</p>
</section>

<% if (memberId == null) { %>
  <main class="content-card">
    <div class="message">請先登入後再進入會員中心。</div>
    <a class="btn-link" href="login.jsp">前往登入</a>
    <a class="btn-link" href="register.jsp">註冊會員</a>
  </main>
<% } else { %>
  <main class="member-grid">
    ...
    <section id="profile" class="member-panel">
      <h2>會員資料</h2>
      <p><strong>會員編號：</strong><%= memberId %></p>
      <p><strong>姓名：</strong><%= mName == null ? "" : mName %></p>
      <p><strong>Email：</strong><%= mEmail == null ? "" : mEmail %></p>
      <p><strong>電話：</strong><%= mPhone == null ? "" : mPhone %></p>
      <p><strong>地址：</strong><%= mAddress == null ? "" : mAddress %></p>
    </section>
  </main>
<% } %>
